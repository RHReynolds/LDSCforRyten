...
## Creating gene lists
```{r Creating gene lists, echo = T, eval = T}
...
```{r Creating annotations, eval = F, echo = T}
...
```{bash LDSC arguments, eval = F, echo = T, tidy = T}
...
```{r results display, eval = T, echo = F}
# # Saved once
# write_csv(x = results %>%
...
## Plotting the results and interpreting them
...
```{r summary LDSC figure pred 1, fig.height = 12, out.width = '100%', echo = F, fig.cap = "***Figure:*** *Results of LDSC with prediction quality >= 1, ordered by co-efficient p-value. Black dashed line indicates the cut-off for Bonferroni significance.*"}

ggplot(data = results %>% 
         dplyr::filter(prediction_quality == "prediction.100") %>% 
         dplyr::filter(!GWAS == "Anxiety2016.fs"), 
...
# # Using enrichment p-value
# ggplot(data = results %>% 
#          dplyr::filter(prediction_quality == "prediction.100"), 
#        aes(x = MarkerGenes::reorder_within(x = panel,
#                                            by = -log10(Enrichment_p),
#                                            within = GWAS,
#                                            fun = max,
#                                            desc = TRUE),
...
#   coord_flip() +
#   facet_wrap(~ GWAS, scales = "free_y", ncol = 2) +
#   labs(x = "", y = expression("Enrichment P-value (-log"[1][0]~"p)"), title = "") +
#   theme_bw() + 
#   theme(axis.title = element_text(size = 8, face = "bold"),
#         axis.text.x = element_text(size = 6, angle = 45, hjust = 1, vjust = 1),
#         strip.text = element_text(size = 6),
...
- Significant results include:
    - Significant enrichment of intelligence heritability in intellectual disability panel. Biologically this makes a lot of sense. 
    - Note that second highest co-efficient p-value (albeit insignificant) for intelligence is achieved with the genetic epilepsy syndromes panel. The Brainstorm paper (https://science.sciencemag.org/content/360/6395/eaap8757.long - see Figure 4) finds that epilepsy and focal epilepsy are negatively correlated with intelligence (epilepsy, correlation = -0.32, p-value = 0.000019239; focal epilepsy, correlation = -0.47, p-value = 0.0017). Generalised epilepsy was not genetically correlated with intelligence.
    - Significant enrichment of SCZ heritability in genetic epilepsy syndromes panel. The Brainstorm paper finds no genetic correlation between all forms of epilepsy and schizophrenia (epilepsy, correlation = -0.03, p-value = 0.6798; focal epilepsy, correlation = -0.0102, p-value = 0.9114; generalised epilepsy, correlation = -0.0249, 0.6328). Note: these genetic correlations were performed with epilepsy GWAS summary stats from 2014 (as opposed to the newest epilepsy GWAS from 2018 that we are using).  
    - Worth noting that with both the intelligence and SCZ GWASs, the top two panels are intellectual disability and genetic epilepsy syndromes. Given overlap between the two panels (30%) question is how much the overlap between these two panels drives the observed signal. 
- Other observations:
    - Looking at the ILAE epilepsy GWASs, the genetic epilepsy syndromes panel does not perform particularly well i.e. ranking by co-efficient p-value places the panel in the bottom 6 for each of the epilepsy GWASs.
